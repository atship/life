1，Application.systemLanguage 获取系统语言（win/mac/linux下可用，ios不可用，会传回unknown）
...
2，资源释放：
    Destroy()可以释放一个实体，它不会释放原始资源（即模型文件，贴图文件，prefab文件等）
    DestroyImmediate()将会释放实体并资源，如果第二个参数传true，编辑器中将会删除原始资源，实际运行未知。